...
exports.getIdHandler = asyncWrapper(async (req, res) => {
  ...
  console.log('String has been decrypted', decrypted);
...
const parseCode = (codeString) => {
  logger(6, 'Service Parsing Decrypted String')
  console.log('raw string in parse method ', codeString)
...
function filterData(string, gameData) {
  ...
  logger(8, 'Receiving Parced String From Parse Method ')
  ...
  logger(9, 'Fetching And Filtering The Game Data For Player')
  gameData.modes = gameData.modes.filter(mode => {
    if (mode.key in modeObject) {
      console.log('including mode ' + mode.name)
    } else {
